...
public class DonHangService {
    ...
    public boolean hasUserPurchasedProduct(NguoiDung user, Integer productId) {
        // Bolt ⚡: Replacing in-memory loops and N+1 queries with a single database query
        if (user == null || user.getId_nguoidung() == null || productId == null) {
            return false;
        }
        return chiTietRepository.existsByUserAndProductAndCompletedStatus(user.getId_nguoidung(), productId);
    }
