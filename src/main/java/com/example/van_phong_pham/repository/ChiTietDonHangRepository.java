...
public interface ChiTietDonHangRepository extends JpaRepository<ChiTietDonHang, Integer> {
    ...
    @Query("SELECT COUNT(ct) > 0 FROM ChiTietDonHang ct WHERE ct.donHang.nguoiDung.id_nguoidung = :idNguoiDung AND ct.sanPham.id_sanpham = :idSanPham AND LOWER(ct.donHang.trang_thai) = 'hoàn thành'")
    boolean existsByUserAndProductAndCompletedStatus(@Param("idNguoiDung") Integer idNguoiDung, @Param("idSanPham") Integer idSanPham);
}
