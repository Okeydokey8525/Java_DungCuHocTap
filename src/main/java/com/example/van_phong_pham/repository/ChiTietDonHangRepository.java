...
public interface ChiTietDonHangRepository extends JpaRepository<ChiTietDonHang, Integer> {
    ...
    // Kiểm tra người dùng đã mua sản phẩm và đơn hàng đã hoàn thành chưa
    @Query("SELECT CASE WHEN COUNT(ct) > 0 THEN true ELSE false END FROM ChiTietDonHang ct WHERE ct.donHang.nguoiDung = :nguoiDung AND ct.sanPham.id_sanpham = :id_sanpham AND ct.donHang.trang_thai = 'Hoàn thành'")
    boolean existsByNguoiDungAndSanPhamAndTrangThai(@Param("nguoiDung") NguoiDung nguoiDung, @Param("id_sanpham") Integer id_sanpham);
}
