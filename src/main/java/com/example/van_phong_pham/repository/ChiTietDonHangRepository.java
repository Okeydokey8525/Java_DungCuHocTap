...
public interface ChiTietDonHangRepository extends JpaRepository<ChiTietDonHang, Integer> {
    ...
    @Query("SELECT COUNT(ct) FROM ChiTietDonHang ct WHERE ct.donHang.nguoiDung = :user AND ct.donHang.trang_thai = 'Hoàn thành' AND ct.sanPham.id_sanpham = :productId")
    long countCompletedPurchases(@Param("user") NguoiDung user, @Param("productId") Integer productId);
}
