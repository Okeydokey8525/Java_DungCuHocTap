...
public interface ChiTietDonHangRepository extends JpaRepository<ChiTietDonHang, Integer> {
    ...
    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM ChiTietDonHang c " +
           "WHERE c.donHang.nguoiDung = :nguoiDung " +
           "AND c.sanPham.id_sanpham = :idSanPham " +
           "AND LOWER(c.donHang.trang_thai) = 'hoàn thành'")
    boolean existsByNguoiDungAndSanPhamAndTrangThaiHoanThanh(
